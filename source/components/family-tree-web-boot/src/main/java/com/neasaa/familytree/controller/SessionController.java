...
import com.neasaa.familytree.webutils.AppSessionWebWrapper;
import com.neasaa.familytree.webutils.HttpSessionUtils;
import com.neasaa.familytree.webutils.WebUtils;
...
public class SessionController {
  ...
  public ResponseEntity<LoginResponse> login(
      @RequestBody LoginRequest loginRequest, HttpServletRequest aRequest) throws Exception {

    log.info("Processing login request for " + loginRequest.getLoginName());
    HttpSession httpSession = aRequest.getSession(false);
    if (httpSession != null) {
      AppSessionWebWrapper existingAppSession =
          HttpSessionUtils.getAppSessionFromHttpSession(httpSession);
  ...
  @ResponseBody
  public ResponseEntity<EmptyOperationResponse> logout(HttpServletRequest aRequest)
      throws Exception {
    HttpSession httpSession = aRequest.getSession();
    return logoutUserFromDb(SessionExitCode.USER_LOGOUT, httpSession);
  }

  private static ResponseEntity<EmptyOperationResponse> logoutUserFromDb(
      SessionExitCode aSessionExitCode, HttpSession aHttpSession) {
    LogoutRequest input = new LogoutRequest();
    input.setSessionExitCode(aSessionExitCode);

    ResponseEntity<EmptyOperationResponse> operationResponse =
        WebRequestHandler.processRequest(LogoutOperation.class, input);

    AppSessionWebWrapper appSessionWrapper =
        HttpSessionUtils.getAppSessionFromHttpSession(aHttpSession);
  ...
  public ResponseEntity<GetSessionDetailResponse> getSessionDetail() throws Exception {
    return WebRequestHandler.processRequest(
        GetSessionDetailOperation.class, new EmptyOperationRequest());
  }
  ...
  //	@ResponseBody
  //	public ResponseEntity<OperationResponse<SessionValidResult>> isSessionValid ( ) throws
  // Exception {
  //
  //		ResponseEntity<OperationResponse<SessionValidResult>> operationResponse =
  // OperationExecutor.executeOperation( ValidateSessionOperation.class, new EmptyDto() );
  //		return operationResponse;
  //	}

  @RequestMapping(value = "/changepassword")
  @ResponseBody
  public ResponseEntity<EmptyOperationResponse> changePwd(
  ...
  @ResponseBody
  public ResponseEntity<RequestForgotPasswordOTPResponse> requestForgotPasswordOTP(
      @RequestBody RequestForgotPasswordOTPRequest request) throws Exception {
  ...
  @ResponseBody
  public ResponseEntity<EmptyOperationResponse> resetForgotPassword(
      @RequestBody ResetForgotPasswordRequest request) throws Exception {
    return WebRequestHandler.processRequest(ResetForgotPasswordOperation.class, request);
  }

  @RequestMapping(value = "/requestSignupOtp")
  @ResponseBody
  public ResponseEntity<RequestSignUpOTPResponse> resetForgotPassword(
      @RequestBody RequestSignUpOTPRequest request) throws Exception {
