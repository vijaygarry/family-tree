...
public class AddressDao extends AbstractDao {
  private static final String SELECT_ADDRESS_BY_ADDRESS_ID =
      "select  ADDRESSID , ADDRESSLINE1 , ADDRESSLINE2 , ADDRESSLINE3 , CITY , DISTRICT , STATE , POSTALCODE , COUNTRY , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "ADDRESS "
          + "where ADDRESSID = ? ";

  private static final String SELECT_ADDRESS_BY_FAMILY_ID =
      "select  A.ADDRESSID , ADDRESSLINE1 , ADDRESSLINE2 , ADDRESSLINE3 , CITY , DISTRICT , STATE , POSTALCODE , COUNTRY , A.CREATEDBY , A.CREATEDDATE , A.LASTUPDATEDBY , A.LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "ADDRESS A, "
          + BASE_SCHEMA_NAME
  ...
  private static String DELETE_ADDRESS_BY_ADDRESS_ID =
      "DELETE FROM " + BASE_SCHEMA_NAME + "ADDRESS WHERE ADDRESSID = ?";
  ...
  public AddressEntity getAddressById(int addressId) {
    return getJdbcTemplate()
        .queryForObject(SELECT_ADDRESS_BY_ADDRESS_ID, new AddressRowMapper(), addressId);
  }

  public AddressEntity getAddressByFamilyId(int familyId) {
    return getJdbcTemplate()
        .queryForObject(SELECT_ADDRESS_BY_FAMILY_ID, new AddressRowMapper(), familyId);
  }

  private PreparedStatement buildInsertStatement(Connection aConection, AddressEntity aAddress)
      throws SQLException {
    String sqlStatement =
        "INSERT INTO "
            + BASE_SCHEMA_NAME
            + "ADDRESS (ADDRESSLINE1, ADDRESSLINE2, ADDRESSLINE3, CITY, DISTRICT, STATE, POSTALCODE, COUNTRY, CREATEDBY, CREATEDDATE, LASTUPDATEDBY, LASTUPDATEDDATE) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    PreparedStatement prepareStatement =
        aConection.prepareStatement(sqlStatement, new String[] {"addressid"});
          ...
          .update(
              new PreparedStatementCreator() {
                ...
                public PreparedStatement createPreparedStatement(Connection aConection)
                    throws SQLException {
                  PreparedStatement prepareStatement =
                      aConection.prepareStatement(DELETE_ADDRESS_BY_ADDRESS_ID);
  ...
  public PreparedStatement buildUpdateStatement(Connection aConection, AddressEntity aAddress)
      throws SQLException {
