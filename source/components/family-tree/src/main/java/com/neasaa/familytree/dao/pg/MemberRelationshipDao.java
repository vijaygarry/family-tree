...
public class MemberRelationshipDao extends AbstractDao {
  ...
  private static final String SELECT_RELATIONSHIP_BY_ID =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
          + "where MEMBERID = ? ";

  private static final String SELECT_RELATIONSHIP_BY_ID_AND_RELATION_TYPE =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
  ...
  private static final String SELECT_RELATIONSHIP_BY_RELATED_ID_AND_RELATION_TYPE =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
  ...
  private static final String SELECT_RELATIONSHIP_BETWEEN_MEMBERS =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID , CREATEDBY , CREATEDDATE , LASTUPDATEDBY , LASTUPDATEDDATE  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
  ...
  private static final String SELECT_SPOUSE_FOR_FAMILY_MEMBER_BY_ID =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
          + "where relationshiptype IN ('Husband', 'Wife') and (memberId = ? or relatedmemberid = ?)";

  private static final String SELECT_CHILDREN_FOR_MEMBER_BY_ID =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
  ...
  // This query will return only one parent for a family member. Other parent can be found by
  // finding spouse of first parent.
  private static final String SELECT_PARENTS_FOR_FAMILY_MEMBER_BY_ID =
      "select  MEMBERID , RELATIONSHIPTYPE , RELATEDMEMBERID  "
          + "from "
          + BASE_SCHEMA_NAME
          + "MEMBERRELATIONSHIP "
  ...
  /**
   * This method returns all the relationships define with member id or related member id. This
   * method is called from GetMemberProfile API to get all the relationships for selected member.
   *
   * @param memberId
   * @return
   */
  public List<MemberRelationshipEntity> getRelationshipByMemberIdOrRelatedMemberId(int memberId) {
    return getJdbcTemplate()
        .query(
            SELECT_RELATIONSHIP_BETWEEN_MEMBERS,
            new MemberRelationshipRowMapper(),
            memberId,
            memberId);
  }

  public List<MemberRelationshipEntity> getRelatedMembersByIdAndRelationType(
      List<Integer> memberIds, List<RelationshipType> relationshipTypes) {
    return getJdbcTemplate()
        .query(
            new PreparedStatementCreator() {
              ...
              public PreparedStatement createPreparedStatement(Connection connection)
                ...
                PreparedStatement ps =
                    connection.prepareStatement(SELECT_RELATIONSHIP_BY_ID_AND_RELATION_TYPE);
                // Convert list to SQL array
                Array memberIdArray = connection.createArrayOf("INTEGER", memberIds.toArray());
                Array relationshipTypeArray =
                    connection.createArrayOf("VARCHAR", relationshipTypes.toArray());
  ...
  public List<MemberRelationshipEntity> getRelationByRelatedIdAndRelationType(
      int relatedMemberId, List<RelationshipType> relationshipTypes) {
    return getJdbcTemplate()
        .query(
            new PreparedStatementCreator() {
              @Override
              public PreparedStatement createPreparedStatement(Connection connection)
                ...
                PreparedStatement ps =
                    connection.prepareStatement(
                        SELECT_RELATIONSHIP_BY_RELATED_ID_AND_RELATION_TYPE);
                // Convert list to SQL array
                Array relationshipTypeArray =
                    connection.createArrayOf("VARCHAR", relationshipTypes.toArray());
  ...
  public MemberRelationshipEntity getRelationshipBetweenMembers(int memberId, int relatedMemberId) {
    List<MemberRelationshipEntity> relationshipList =
        getJdbcTemplate()
            .query(
                SELECT_RELATIONSHIP_BETWEEN_MEMBERS,
                new MemberRelationshipRowMapper(),
  ...
  public List<MemberRelationshipEntity> getParentsForMemberById(int memberId) {
    List<MemberRelationshipEntity> relationshipList =
        getJdbcTemplate()
            .query(
                SELECT_PARENTS_FOR_FAMILY_MEMBER_BY_ID,
                new MemberRelationshipRowMapper(),
                memberId);
    ...
    MemberRelationshipEntity spouseForFirstParent =
        getSpouseForMemberById(firstParent.getMemberId());
    if (spouseForFirstParent != null) {
      // getSpouseForMemberById returns relatedMemberId as spouse member id, so create new
      // relationship for second parent
      MemberRelationshipEntity secondParent =
          MemberRelationshipEntity.builder()
              .memberId(spouseForFirstParent.getRelatedMemberId())
              .relationshipType(
                  firstParent
                      .getRelationshipType()) // both parents will have same relationship type
  ...
  /**
   * This method returns spouse for a family member. Return relationship will have memberId as
   * specified member id and relatedMemberId as spouse member id. If spouse is not found, it returns
   * null.
   *
   * @param memberId
   * @return
  ...
  public MemberRelationshipEntity getSpouseForMemberById(int memberId) {
    List<MemberRelationshipEntity> relationshipList =
        getJdbcTemplate()
            .query(
                SELECT_SPOUSE_FOR_FAMILY_MEMBER_BY_ID,
                new MemberRelationshipRowMapper(),
                memberId,
                memberId);
            ...
            .query(
                new PreparedStatementCreator() {
                  ...
                  public PreparedStatement createPreparedStatement(Connection connection)
                      throws SQLException {
                    PreparedStatement ps =
                        connection.prepareStatement(SELECT_CHILDREN_FOR_MEMBER_BY_ID);
                    // Convert list to SQL array
                    Array memberIdArray = connection.createArrayOf("INTEGER", memberIds.toArray());
                    ps.setArray(1, memberIdArray);
                    return ps;
  ...
  /**
   * This method is called recursively to build family tree from GetFamilyDetails API It returns all
   * the relationships for a member.
   *
   * @param memberId
   * @return
   */
  public List<MemberRelationshipEntity> getRelationshipsForMember(int memberId) {
    return getJdbcTemplate()
        .query(SELECT_RELATIONSHIP_BY_ID, new MemberRelationshipRowMapper(), memberId);
  }

  public int addMemberRelationship(
      MemberRelationshipEntity aMemberRelationship, AuditInfo auditInfo) {
    try {
      return getJdbcTemplate()
          .update(
              new PreparedStatementCreator() {
      ...
      log.error(
          "Error while adding member relationship for memberId: "
              + aMemberRelationship.getMemberId()
              + ", relatedMemberId: "
              + aMemberRelationship.getRelatedMemberId()
              + ", relationshipType: "
              + aMemberRelationship.getRelationshipType(),
          ex);
  ...
  private PreparedStatement buildInsertStatement(
      Connection aConection, MemberRelationshipEntity aMemberRelationship, AuditInfo auditInfo)
      throws SQLException {
    String sqlStatement =
        "INSERT INTO "
            + BASE_SCHEMA_NAME
            + "MEMBERRELATIONSHIP (MEMBERID, RELATIONSHIPTYPE, RELATEDMEMBERID, CREATEDBY, CREATEDDATE, LASTUPDATEDBY, LASTUPDATEDDATE) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  public int deleteMemberRelationship(MemberRelationshipEntity aMemberRelationship)
      throws SQLException {
    return getJdbcTemplate()
        .update(
            new PreparedStatementCreator() {
              ...
              public PreparedStatement createPreparedStatement(Connection aConection)
                ...
                String deleteSqlQuery =
                    "DELETE FROM MEMBERRELATIONSHIP WHERE MEMBERID = ? and RELATEDMEMBERID = ?";
  ...
  public PreparedStatement buildUpdateStatement(
      Connection aConection, MemberRelationshipEntity aMemberRelationship) throws SQLException {
    String updateStatement =
        "UPDATE MEMBERRELATIONSHIP SET RELATIONSHIPTYPE = ? , CREATEDBY = ? , CREATEDDATE = ? , LASTUPDATEDBY = ? , LASTUPDATEDDATE = ?  where MEMBERID = ? and RELATEDMEMBERID = ?";
  ...
  public int updateMemberRelationship(MemberRelationshipEntity aMemberRelationship)
      throws SQLException {
    return getJdbcTemplate()
        .update(
            new PreparedStatementCreator() {
