...
public class ManageRelationshipOperation
  ...
  public void doValidate(ManageRelationshipRequest opRequest) throws OperationException {
    if ((opRequest.getToAdd() == null || opRequest.getToAdd().isEmpty())
        && (opRequest.getToRemove() == null || opRequest.getToRemove().isEmpty())) {
  ...
  private void validateRelationship(RelationshipDto relationship) {
    ...
    if (relationship.getMemberId() <= 0) {
      throw new ValidationException(
          "Invalid member ID " + relationship.getMemberId() + " provided.");
    }
    if (relationship.getRelatedMemberId() <= 0) {
      throw new ValidationException(
          "Invalid related member ID " + relationship.getRelatedMemberId() + " provided.");
    }
  ...
  @Override
  public ManageRelationshipResponse doExecute(ManageRelationshipRequest opRequest)
                ...
                response
                    ...
                    .add(
                        "Relationship between member "
                            + relationshipDto.getMemberName()
                            + " and related member name "
                            + relationshipDto.getRelatedMemberName()
  ...
  private void removeRelationship(RelationshipDto relationship) {
    ...
    MemberRelationshipEntity relationshipBetweenMembers =
    ...
    if (relationshipBetweenMembers == null) {
      log.info(
          "No relationship found between member ID {} and related member ID {}. Nothing to remove.",
    ...
    if (relationshipBetweenMembers.getRelationshipType() == relationshipType) {
      log.info(
          "Removing relationship between member ID {} and related member ID {} with relationship type {}",
          relationship.getMemberId(),
          relationship.getRelatedMemberId(),
          relationshipType);
      try {
        ...
        throw new RuntimeException(
            "Internal error while removing relationship between member ID "
                + relationship.getMemberId()
                + " and related member ID "
                + relationship.getRelatedMemberId());
      }
    } else {
      log.warn(
          "Relationship type mismatch. Expected: {}, Found: {}. Not removing relationship.",
          relationshipType,
          relationshipBetweenMembers.getRelationshipType());
  ...
  private void addRelationship(RelationshipDto relationship) {
    ...
    if (relationshipBetweenMembers == null) {
      ...
      MemberRelationshipEntity memberRelationship =
      ...
      log.info(
          "Added relationship between member ID {} and related member ID {} with relationship type {}",
          relationship.getMemberId(),
          relationship.getRelatedMemberId(),
          relationshipType);
    } else {
      log.info(
          "Relationship already exists between member ID {} and related member ID {} with relationship type {}",
          relationship.getMemberId(),
          relationship.getRelatedMemberId(),
          relationshipType);
    }
  ...
  private FamilyMemberEntity getAndValidateMember(int memberId, String memberName) {
    ...
    if (!familyMember.getFirstName().equalsIgnoreCase(memberName)) {
      throw new ValidationException(
          "Member name "
              + memberName
              + " does not match with the provided member name "
              + memberName);
    }
