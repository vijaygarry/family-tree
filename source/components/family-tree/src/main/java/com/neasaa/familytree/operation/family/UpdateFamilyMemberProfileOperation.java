...
@Scope("prototype")
public class UpdateFamilyMemberProfileOperation
    extends FamilyAbstractOperation<
        UpdateFamilyMemberProfileRequest, UpdateFamilyMemberProfileResponse> {
  ...
  public UpdateFamilyMemberProfileResponse doExecute(UpdateFamilyMemberProfileRequest opRequest)
    ...
    if (!canLoggedInUserUpdateMember(memberEntityFromDb.getFamilyId())) {
      log.info(
          "User is not allowed to update the member details for member id: {}",
    ...
    AddressEntity memberAddressFromDb =
        addressDao.getAddressById(memberEntityFromDb.getMemberAddressId());
    // If user selected address same as family, but existing address is different, then delete the
    // existing address
    if (opRequest.isAddressSameAsFamily()) {
      ...
      if (memberEntityFromDb.getMemberAddressId()
          == Constants.MEMBER_ADDRESS_SAME_AS_FAMILY_ADDRESS) {
    ...
    MemberProfileDto memberProfile =
        MemberProfileDto.fromFamilyMemberDBEntity(
            newFamilyMemberEntity, memberAddressDto, null, true);

    UpdateFamilyMemberProfileResponse response =
        UpdateFamilyMemberProfileResponse.builder().memberProfile(memberProfile).build();
    ...
    response.setOperationMessage(
        String.format(
            "Member %s %s updated successfully !!!",
            opRequest.getFirstName(), newFamilyMemberEntity.getLastName()));
    return response;
  }

  private void deleteAddressIfNotLinkedToMember(
      UpdateFamilyMemberProfileRequest opRequest, FamilyMemberEntity memberEntityFromDb) {
    if (opRequest.isAddressSameAsFamily()) {
      if (memberEntityFromDb.getMemberAddressId()
          != Constants.MEMBER_ADDRESS_SAME_AS_FAMILY_ADDRESS) {
